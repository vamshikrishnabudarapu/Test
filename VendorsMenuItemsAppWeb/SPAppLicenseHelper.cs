...
namespace VendorsMenuItemsAppWeb
{
    /// <summary>
    ...
    public class SPAppLicenseHelper
    {
            ...
            else
            {
                ...
                switch (licenseType)
                {
                    case SPAppLicenseType.Free:
                        uiWarning = String.Format(GetLicenseStringFromResource(_freeStringKey), appDisplayName, "<a href='" + GetStoreSearchUrl(appDisplayName, hostWebUrl, currentPageUrl) + "'>", "</a>");          
                        break;
                    ...
                    case SPAppLicenseType.Trial:
                        ...
                        if (remainingDays == int.MaxValue)
                        {
                            ...
                            uiWarning = String.Format(GetLicenseStringFromResource(_trialUnlimitedStringKey), appDisplayName, "<a href='" + GetStoreSearchUrl(appDisplayName, hostWebUrl, currentPageUrl) + "'>", "</a>", " ");
                        }
                        else
                        {
                            //Time limited trial
                            if (remainingDays > 0)
                            {
                                //uiWarning = String.Format(GetLicenseStringFromResource(_trialStringKey), appDisplayName, remainingDays, "<a href='" + storeFrontUrl + "'>", "</a>", "test feature");
                                uiWarning = String.Format(GetLicenseStringFromResource(_trialStringKey), appDisplayName, remainingDays, "<a href='" + GetStoreSearchUrl(appDisplayName, hostWebUrl, currentPageUrl) + "'>", "</a>", " ");
                                //uiWarning = String.Format(GetLicenseStringFromResource(_trialStringKey), appDisplayName, remainingDays);
                            }
                            else
                            {
                                //Expired trial
                                uiWarning = String.Format(GetLicenseStringFromResource(_trialExpiredStringKey), "<a href='" + GetStoreSearchUrl(appDisplayName, hostWebUrl, currentPageUrl) + "'>", "</a>");
                            }        
                        }
        ...
        public static string GetVersionPeriodApp(VerifyEntitlementTokenResponse verifiedLicense, string hostWebUrl, string currentPageUrl, string appDisplayName)
        {
            string AppPeriod = String.Empty;
                    ...
                    case SPAppLicenseType.Trial:
                        int remainingDays = GetRemainingDays(verifiedLicense);
                            ...
                            if (remainingDays > 0)
                            {
                                ...
                                AppPeriod = "TrialRemaining";
                            }
                            else
                            {
                                //Expired trial
                                AppPeriod = "TrialExpired";
                            }
        ...
        public static string GetStoreSearchUrl(string appName, string hostWebUrl, string currentPageUrl)
        {
            ...
            return String.Format(storeSearchTemplateString, hostWebUrl, currentPageUrl, appName, appName+"  debopam");  
